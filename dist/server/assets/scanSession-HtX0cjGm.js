...
function storeScanSession(contact, imageDataUrl, meta) {
...
function loadScanSession() {
  ...
  try {
    ...
    return { contact: JSON.parse(raw), imageDataUrl, meta };
  } catch {
    return { contact: null, imageDataUrl, meta: null };
  }
}
function isValidCardImage(file) {
...
async function dataUrlToFile(dataUrl, fileName = "scan.jpg") {
  const response = await fetch(dataUrl);
  const blob = await response.blob();
  const type = blob.type || "image/jpeg";
  return new File([blob], fileName, { type });
}
function isGarbageExtractedValue(value) {
...
function isEmptyScanContact(contact) {
...
export {
  isEmptyScanContact as a,
  dataUrlToFile as d,
  isValidCardImage as i,
  loadScanSession as l,
  readFileAsDataUrl as r,
  storeScanSession as s
};

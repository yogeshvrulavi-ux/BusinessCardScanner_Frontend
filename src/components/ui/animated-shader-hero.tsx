...
export interface AnimatedShaderBackgroundProps {
  className?: string;
  /** When true, sizes to parent container instead of the window. */
  contained?: boolean;
}
...
class WebGLRenderer {
  ...
  setup() {
    ...
    this.program = gl.createProgram() as ShaderProgram | null;
...
class PointerHandler {
  ...
  constructor(
    ...
    const map = (canvas: HTMLCanvasElement, s: number, clientX: number, clientY: number) => {
      const rect = canvas.getBoundingClientRect();
      const x = clientX - rect.left;
      const y = clientY - rect.top;
      return [x * s, canvas.height - y * s];
    };
...
function useShaderBackground(
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      const target = contained ? containerRef.current : null;
      dpr = Math.max(1, 0.5 * window.devicePixelRatio);

      const width = target ? target.clientWidth : window.innerWidth;
      const height = target ? target.clientHeight : window.innerHeight;
    ...
    if (rendererRef.current.test(DEFAULT_SHADER_SOURCE) === null) {
      rendererRef.current.updateShader(DEFAULT_SHADER_SOURCE);
    }
  ...
  }, [containerRef, contained]);

  return canvasRef;
}

/** Shader canvas only — for auth panels and embedded backgrounds. */
export function AnimatedShaderBackground({
  className,
  contained = true,
}: AnimatedShaderBackgroundProps) {
  const containerRef = useRef<HTMLDivElement>(null);
  const canvasRef = useShaderBackground(containerRef, contained);
...
/** Full-screen hero with animated shader background. */
export function AnimatedShaderHero({
  ...
  const containerRef = useRef<HTMLDivElement>(null);
  const canvasRef = useShaderBackground(containerRef, false);
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 z-10 flex flex-col items-center justify-center text-white">
        {trustBadge ? (
          <div className="mb-8 animate-shader-hero-fade-in-down">
        ...
        <div className="mx-auto max-w-5xl space-y-6 px-4 text-center">
          <div className="space-y-2">
            <h1 className="animate-shader-hero-fade-in-up shader-hero-delay-200 bg-gradient-to-r from-orange-300 via-yellow-400 to-amber-300 bg-clip-text text-5xl font-bold text-transparent md:text-7xl lg:text-8xl">
              {headline.line1}
            </h1>
            <h1 className="animate-shader-hero-fade-in-up shader-hero-delay-400 bg-gradient-to-r from-yellow-300 via-orange-400 to-red-400 bg-clip-text text-5xl font-bold text-transparent md:text-7xl lg:text-8xl">
              {headline.line2}
            </h1>
          </div>

          <div className="animate-shader-hero-fade-in-up shader-hero-delay-600 mx-auto max-w-3xl">
          ...
          {buttons ? (
            <div className="animate-shader-hero-fade-in-up shader-hero-delay-800 mt-10 flex flex-col justify-center gap-4 sm:flex-row">
              {buttons.primary ? (
                <button
                  type="button"

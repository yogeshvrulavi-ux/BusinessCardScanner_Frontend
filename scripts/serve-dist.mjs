/**
 * Serve the production build (dist/server + dist/client) locally.
 * TanStack Start has no index.html — routes are rendered by the SSR handler.
 */
...
if (!existsSync(join(root, "dist", "server", "server.js"))) {
  console.error("Missing dist/server/server.js — run: npm run build");
  process.exit(1);
}
...
if (typeof handler !== "function") {
  console.error("dist/server/server.js must export default { fetch }");
  process.exit(1);
}
...
httpServer.on("error", (err) => {
  if (err.code === "EADDRINUSE") {
    console.error(
      `Port ${port} is already in use (another serve:dist or vite preview may be running).`,
    );
    console.error(`  Use another port:  $env:PORT=4174; npm run serve:dist`);
    console.error(`  Or stop the old process, then run npm run serve:dist again.`);
    process.exit(1);
  }
  throw err;
});
